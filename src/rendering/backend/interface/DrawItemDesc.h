...
static size_t GetDrawItemSize(const DrawItemDesc& desc) {
    return sizeof(size_t) // total item size
        ...
        + sizeof(size_t) // binding count
        + sizeof(Binding) * desc.bindingCount
        + sizeof(size_t) // stream count
        + sizeof(VertexStream) * desc.streamCount
        + (desc.drawCall.type == DrawCall::Type::Indexed ? sizeof(BufferId) + sizeof(uint32_t) : 0); // indexBuffer and offset
}
}